...
class World:
    ...
    def is_colliding(self, rect, ignore_entities=None):
        """Проверяет, пересекается ли rect с объектами мира и с сущностями (игрок, NPC)"""
        ...
        for obj in self.objects:
            if obj["type"] in ("building", "house"):
                if obj["rect"].colliderect(rect):
                    return True
        # Проверка сущностей (если они добавлены в менеджер) – здесь мы можем получить доступ к EntityManager
        # Пока оставим заглушку, так как циклическая зависимость
        return False
