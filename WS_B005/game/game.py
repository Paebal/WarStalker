...
class Game:
    def __init__(self, settings):
        ...
        self.player = Player(spawn_x, spawn_y, self.config_loader)
    ...
    def new_game(self):
        ...
        sx, sy = self.get_spawn()
        self.player = Player(sx, sy, self.config_loader)
        self.entity_manager.set_player(self.player)
        self.bullets = []
        self.chat = Chat()
        self.hud = HUD(self.player, self.constants)
    ...
    def load_game(self, filename="save.json"):
        try:
            ...
            self.player = Player(0, 0, self.config_loader)
