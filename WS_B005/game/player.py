...
class Player(Entity):
    def __init__(self, x, y, config_loader):
        ...
        self.stamina_drain_running = 2.0      # изменено с 1.0 на 2.0
    ...
    def update(self, dt, keys_pressed, settings):
        ...
        right_vec = pygame.math.Vector2(dir_vec.y, -dir_vec.x)
        ...
        if is_moving and move_vec.length() > 0:
            if self.stamina <= 0:
                ...
                self.x += move_vec.x * speed * dt
                self.y += move_vec.y * speed * dt
            elif is_walking:
